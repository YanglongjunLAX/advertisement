...
@implementation UIDevice (BOAD)

- (NSString *)BOAD_sysInfoByName:(char *)typeSpecifier {
	size_t size;
...
- (NSString *)BOAD_hostname {// retun the host name
...
- (NSString *)BOAD_findIPAddress:(BOOL)preferIPv4
{
    NSArray *searchArray = preferIPv4 ?
    @[ IOS_WIFI @"/" IP_ADDR_IPv4, IOS_WIFI @"/" IP_ADDR_IPv6, IOS_CELLULAR @"/" IP_ADDR_IPv4, IOS_CELLULAR @"/" IP_ADDR_IPv6 ] :
    @[ IOS_WIFI @"/" IP_ADDR_IPv6, IOS_WIFI @"/" IP_ADDR_IPv4, IOS_CELLULAR @"/" IP_ADDR_IPv6, IOS_CELLULAR @"/" IP_ADDR_IPv4 ] ;

    NSDictionary *addresses = [self BOAD_findIPAddresses];
...
- (NSDictionary *)BOAD_findIPAddresses
{
...
- (NSString *)BOAD_model {
	return [self BOAD_sysInfoByName:"hw.machine"];
}

- (NSString *)BOAD_macAddress {
...
- (NSString *)BOAD_ipAddress {
     ...
     */
    return [self BOAD_findIPAddress:YES];
}

- (NSString *)BOAD_uuid {
	if ([[[UIDevice currentDevice] systemVersion] compare:@"6" options:NSNumericSearch] == NSOrderedAscending) {
		NSString *macAddress = [self BOAD_macAddress];
		NSString *uuid = [macAddress BOAD_md5];
		return uuid;
	}
	else {
